...
namespace NiceIO
{
    ...
    /// </remarks>
    public partial class NPath : IEquatable<NPath>, IComparable
    {
        ...
        private static bool IsWindows { get; } = Environment.OSVersion.Platform == PlatformID.Win32Windows || Environment.OSVersion.Platform == PlatformID.Win32NT;
        ...
        private string MakeCompletelyWellFormatted(string path, bool doubleDotsAreCollapsed = false)
        {
            if (path == ".")
                return ".";
            if (path.Length == 0)
            ...
            for (int i = 0; i != path.Length; i++)
            {
                ...
                if (c == '.' && previousChar == '.' && !doubleDotsAreCollapsed)
                    return MakeCompletelyWellFormatted(CollapseDoubleDots(path), true);

                if (c == '.' && (IsSlash(previousChar) || previousChar == '\0') && (IsSlash(nextChar) || nextChar == '\0'))
                    return MakeCompletelyWellFormatted(CollapseSingleDots(path));

                if (IsSlash(c) && IsSlash(previousChar))
        ...
        private string CollapseSingleDots(string path)
        {
        ...
        private string CollapseDoubleSlashes(string path)
        {
            return path.Replace("\\", "/").Replace("//", "/");
        }

        private string CollapseDoubleDots(string path)
        {
            path = path.Replace("\\", "/");
        ...
        public bool IsRelative
        {
            get
            {
                ...
                for (int i = 0; i != _path.Length; i++)
                {
                    ...
                    if (IsSlash(c))
                        return previousChar != '\0' && previousChar != ':';
                    previousChar = c;
                }
        ...
        {
            if (slashMode == SlashMode.Forward || (slashMode == SlashMode.Native && !IsWindows))
                return _path;
        ...
        public override bool Equals(Object obj)
        {
            ...
            // If parameter cannot be cast to NPath return false.
            var p = obj as NPath;
            if ((Object)p == null)
                return false;
        ...
        {
            return string.Equals(p._path, _path, PathStringComparison);
        }
        ...
        // TODO: This isn't ideal on case-insensitive filesystems - two paths that differ only by case will return true
        // TODO: for Equals(), but have different GetHashCode() values.
        ...
        public NPath[] Files(string filter, bool recurse = false)
        {
            ...
            return Directory.GetFiles(ToString(SlashMode.Native), filter, recurse ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly).Select(s => new NPath(s)).ToArray();
        }
        ...
        public NPath[] Files(string[] extensions, bool recurse = false)
        {
            ...
            return Directory.GetFiles(ToString(SlashMode.Native), "*", recurse ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly).Select(s => new NPath(s)).Where(p => extensions.Contains(p.Extension)).ToArray();
        }
        ...
        {
            GlobbingCallback.Invoke(this, filter, recurse);
            return Directory.GetDirectories(ToString(SlashMode.Native), filter, recurse ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly).Select(s => new NPath(s)).ToArray();
        }
        ...
        public NPath CreateFile()
        {
            ...
            File.WriteAllBytes(ToString(SlashMode.Native), new byte[0]);
            return this;
        }
        ...
        public NPath CreateDirectory()
        {
            ...
            Directory.CreateDirectory(ToString(SlashMode.Native));
            return this;
        }

        /// <summary>
        /// Append the given path fragment to this path, and create an empty directory there.
        /// </summary>
        ...
        NPath CopyWithDeterminedDestination(NPath absoluteDestination, Func<NPath, bool> fileFilter)
        {
            ...
            if (FileExists())
            {
                ...
                File.Copy(ToString(SlashMode.Native), absoluteDestination.ToString(SlashMode.Native), true);
        ...
        public void Delete(DeleteMode deleteMode = DeleteMode.Normal)
        {
            ...
            if (FileExists())
                File.Delete(ToString(SlashMode.Native));
            else if (DirectoryExists())
                try
                {
                    Directory.Delete(ToString(SlashMode.Native), true);
                }
                catch (IOException)
                {
        ...
        public NPath Move(NPath dest)
        {
            ...
            if (FileExists())
            {
                ...
                File.Move(ToString(SlashMode.Native), dest.ToString(SlashMode.Native));
                return dest;
            }

            if (DirectoryExists())
            {
                Directory.Move(ToString(SlashMode.Native), dest.ToString(SlashMode.Native));
                return dest;
            }
        ...
        public bool IsChildOf(NPath potentialBasePath)
        {
            ...
            if (IsRelative && potentialBasePath == ".")
        ...
        public IEnumerable<NPath> RecursiveParents
        {
            get
            {
                ...
                while (true)
                {
                    if (candidate.IsRoot || candidate.ToString() == ".")
                        yield break;
        ...
        public NPath WriteAllText(string contents)
        {
            ThrowIfRelative();
            EnsureParentDirectoryExists();
            File.WriteAllText(ToString(SlashMode.Native), contents);
            return this;
        }
        ...
        public string ReadAllText()
        {
            ...
            return File.ReadAllText(ToString(SlashMode.Native));
        }
        ...
        public NPath WriteAllLines(string[] contents)
        {
            ThrowIfRelative();
            EnsureParentDirectoryExists();
            File.WriteAllLines(ToString(SlashMode.Native), contents);
            return this;
        }
        ...
        public string[] ReadAllLines()
        {
            ThrowIfRelative();
            ReadContentsCallback.Invoke(this);
            return File.ReadAllLines(ToString(SlashMode.Native));
        }
        ...
        {
            File.SetLastWriteTimeUtc(ToString(SlashMode.Native), lastWriteTimeUtc);
            return this;
        }
        ...
        {
            return File.GetLastWriteTimeUtc(ToString(SlashMode.Native));
        }
